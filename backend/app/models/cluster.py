...
class Cluster(Base, UUIDMixin, TimestampMixin):
    ...
    api_server_url: Mapped[str | None] = mapped_column(String(500), nullable=True)
    kubernetes_version: Mapped[str | None] = mapped_column(String(50), nullable=True)
    ...
    memory_capacity_gb: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class ClusterNode(Base, UUIDMixin, TimestampMixin):
    ...
    container_runtime: Mapped[str | None] = mapped_column(String(100), nullable=True)
    cpu_allocatable: Mapped[float | None] = mapped_column(Float, nullable=True)
    memory_allocatable_gb: Mapped[float | None] = mapped_column(Float, nullable=True)
...
class KubernetesNamespace(Base, UUIDMixin, TimestampMixin):
    ...
    labels: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=dict)
    annotations: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=dict)
    resource_quota: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class KubernetesWorkload(Base, UUIDMixin, TimestampMixin):
    ...
    cluster_id: Mapped[str] = mapped_column(String(36), ForeignKey("clusters.id", ondelete="CASCADE"), nullable=False)
    namespace_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("kubernetes_namespaces.id", ondelete="SET NULL"), nullable=True)
    ...
    selector: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=dict)
...
class KubernetesReplicaSet(Base, UUIDMixin, TimestampMixin):
    ...
    cluster_id: Mapped[str] = mapped_column(String(36), ForeignKey("clusters.id", ondelete="CASCADE"), nullable=False)
    namespace_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("kubernetes_namespaces.id", ondelete="SET NULL"), nullable=True)
    workload_id: Mapped[str | None] = mapped_column(String(36), ForeignKey("kubernetes_workloads.id", ondelete="SET NULL"), nullable=True)
    namespace_name: Mapped[str] = mapped_column(String(255), nullable=False)
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    owner_kind: Mapped[str | None] = mapped_column(String(100), nullable=True)
...
class KubernetesPod(Base, UUIDMixin, TimestampMixin):
    ...
    pod_ip: Mapped[str | None] = mapped_column(String(100), nullable=True)
    restart_count: Mapped[int] = mapped_column(Integer, default=0)
    ready: Mapped[bool] = mapped_column(Boolean, default=False)
    owner_kind: Mapped[str | None] = mapped_column(String(100), nullable=True)
    owner_name: Mapped[str | None] = mapped_column(String(255), nullable=True)
    containers: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=list)
    labels: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=dict)
    annotations: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=dict)
...
class KubernetesService(Base, UUIDMixin, TimestampMixin):
    ...
    cluster_ip: Mapped[str | None] = mapped_column(String(100), nullable=True)
    external_ip: Mapped[str | None] = mapped_column(String(255), nullable=True)
    ports: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=list)
    selector: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=dict)
    labels: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=dict)
    annotations: Mapped[dict | None] = mapped_column(JSON, nullable=True, default=dict)
